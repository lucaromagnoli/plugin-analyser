...
{
    int signalType = signalTypeCombo.getSelectedId();
    if (signalType == 1) config.signalType = "sine";
    else if (signalType == 2) config.signalType = "noise";
    else if (signalType == 3) config.signalType = "sweep";
    ...
    if (rawCsvButton.getToggleState()) config.analyzers.push_back("RawCsv");
    if (rmsPeakButton.getToggleState()) config.analyzers.push_back("RmsPeak");
    if (transferCurveButton.getToggleState()) config.analyzers.push_back("TransferCurve");
    if (linearResponseButton.getToggleState()) config.analyzers.push_back("LinearResponse");
    if (thdButton.getToggleState()) config.analyzers.push_back("Thd");
}

void MeasurementConfigComponent::loadFromConfig(const Config& config)
{
    if (config.signalType == "sine") signalTypeCombo.setSelectedId(1);
    else if (config.signalType == "noise") signalTypeCombo.setSelectedId(2);
    else if (config.signalType == "sweep") signalTypeCombo.setSelectedId(3);
